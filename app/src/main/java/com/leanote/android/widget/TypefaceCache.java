...
public class TypefaceCache {
    ...
    protected static void setCustomTypeface(Context context, TextView view, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.LeaTextView, 0, 0);
            if (a != null) {
                try {
                    family = a.getInteger(R.styleable.LeaTextView_fontFamily, FAMILY_DEFAULT);
                    variation = a.getInteger(R.styleable.LeaTextView_fontVariation, VARIATION_DEFAULT);
                } finally {
                    a.recycle();
                }
            }
