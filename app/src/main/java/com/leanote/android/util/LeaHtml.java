...
public class LeaHtml {
    ...
    public static Spanned fromHtml(String source, ImageGetter imageGetter,
        ...
        HtmlToSpannedConverter converter = new HtmlToSpannedConverter(source,
                imageGetter, tagHandler, parser, ctx, note, maxImageWidth);
    ...
    private static void withinParagraph(StringBuilder out, Spanned text,
        ...
        for (int i = start; i < end; i = next) {
            ...
            for (int j = 0; j < style.length; j++) {
                ...
                if (style[j] instanceof LeaUnderlineSpan) {
                    out.append("<u>");
                }
                ...
                } else if (style[j] instanceof LeaImageSpan && ((LeaImageSpan) style[j]).getMediaFile().getMediaId() != null) {
                    out.append(getContent((LeaImageSpan) style[j]));
                } else if (style[j] instanceof LeaImageSpan) {
                    out.append("<img src=\"");
                    out.append(((LeaImageSpan) style[j]).getSource());
                    out.append("\" android-uri=\""
                            + ((LeaImageSpan) style[j]).getImageSource()
                            .toString() + "\"");
                    out.append(" />");
    ...
    public static String getContent(LeaImageSpan imageSpan) {
...
class HtmlToSpannedConverter implements ContentHandler {
    ...
    private LeaHtml.ImageGetter mImageGetter;
    ...
    private void startImg(SpannableStringBuilder text, Attributes attributes, LeaHtml.ImageGetter img) {
        ...
        try {
            resizedBitmap = ImageUtils.getLeaImageSpanThumbnailFromFilePath(mContext, src, mMaxImageWidth);
            if (resizedBitmap == null && src != null) {
                if (src.contains("video")) {
                    resizedBitmap = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.media_movieclip);
                } else {
        ...
        if (resizedBitmap != null) {
            ...
            LeaImageSpan is = new LeaImageSpan(mContext, resizedBitmap, curStream);

            // get the MediaFile data from db
            //MediaFile mf = WordPress.wpDB.getMediaFile(src, mNote);
            MediaFile mf = null;
            if (mf != null) {
                is.setMediaFile(mf);
